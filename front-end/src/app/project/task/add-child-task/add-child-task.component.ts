...
import { completedStatus, inProgressStatus, newStatus, priorities, trackers } from 'src/app/utils/variables';
...
export class AddChildTaskComponent implements OnInit {
  ...
  submit() {
    ...
    if (this.taskForm.valid) {
      ...
      // @ts-ignore
      this.dataService.postItem(new Task(), '/addChild/' + this.taskForm.value.parentId, task)
        .pipe()
        .subscribe({
          next: (res) => {
            this.close()
